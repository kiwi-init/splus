...
struct ReviewArgs {
    ...
    #[arg(long)]
    scip: Option<PathBuf>,
    /// Skip cognitive-complexity maintainability metrics (on by default; the
    /// delta-only scoring keeps them quiet on unchanged code).
    #[arg(long)]
    no_metrics: bool,
...
fn run_review(args: ReviewArgs) -> Result<i32> {
    ...
    engine.metrics = !args.no_metrics;
