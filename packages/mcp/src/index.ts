#!/usr/bin/env node
/**
 * @splus/mcp — the Splus MCP server (the FREE, local tier).
 *
 * Exposes the deterministic review engine as Model Context Protocol tools that a
 * local coding agent (Claude Code, Codex, …) calls over stdio. The agent is the
 * LLM here: Splus supplies precise, deterministic findings — each with a
 * provenance anchor + cross-file blast radius — plus the learned per-repo
 * suppression store; the agent reasons over them, surfaces what matters, and
 * applies fixes. No API key, nothing billed to us. That's the whole point of the
 * free tier: the user's own agent does the thinking.
 *
 * Tools:
...
const server = new McpServer({ name: "splus", version: "0.1.0" });
...
/** Map the tool's mode/base inputs to the engine's DiffMode. */
function toMode(mode: string | undefined, base: string | undefined): DiffMode {
  switch (mode) {
    case "staged":
      return { kind: "staged" };
    case "all":
      return { kind: "all" };
    case "base":
      return { kind: "base", ref: base ?? "HEAD~1" };
    default:
      return { kind: "working" };
  }
}
...
/** Recompute summary counts after suppression drops findings (mirrors the CLI). */
function withKept(report: Report, kept: Finding[], suppressedCount: number): Report {
...
server.registerTool(
  "review",
  {
    ...
    description:
      "Run Splus's deterministic review on NEW/changed lines only (clean-as-you-code). " +
      "Returns findings grouped must-fix / concern / nit — each with file:line, rule id, " +
      "severity, confidence, a deterministic provenance anchor, an optional fix, and " +
      "cross-file blast radius (who calls the changed symbol). Per-repo learned suppressions " +
      "are applied first. No LLM runs — YOU are the reviewer: read these findings, surface the " +
      "ones that matter, and apply fixes. Pass a finding's `id` to the `dismiss` tool when the " +
      "user agrees something is noise.",
    inputSchema: {
      root: z
        .string()
        .optional()
  ...
  async ({ root, mode, base, applyLearnings }) => {
    const repo = rootOf(root);
    if (mode === "base" && !base) return fail("mode='base' requires a `base` ref.");

    let report: Report;
    try {
      report = await runEngine({ root: repo, mode: toMode(mode, base) });
    } catch (e) {
      return fail(`Engine error: ${e instanceof Error ? e.message : String(e)}`);
    }
    ...
    if (applyLearnings !== false) {
      try {
        ...
        report = withKept(report, r.kept, r.suppressed.length);
...
server.registerTool(
  "dismiss",
  {
    ...
    description:
      ...
      "forward. Looks the finding up in the current diff to capture its text for matching.",
    inputSchema: {
  ...
  async ({ root, id, mode, base }) => {
    const repo = rootOf(root);
    const store = new FileSuppressionStore(learningsPath(repo));
    let found: Finding | undefined;
    try {
      const report = await runEngine({ root: repo, mode: toMode(mode, base) });
...
server.registerTool(
  ...
  async ({ root, ruleId }) => {
    const store = new FileSuppressionStore(learningsPath(rootOf(root)));
...
server.registerTool(
  "learnings",
  {
    ...
    description:
      ...
      "rules, with the signal and timestamp for each.",
    inputSchema: {
      root: z.string().optional().describe("Repo root (default: server CWD)."),
    },
    annotations: { readOnlyHint: true, openWorldHint: false },
  ...
  async ({ root }) => {
    ...
    if (!entries.length) return ok("No learnings yet. Use the `dismiss` or `mute` tools to teach this repo.");
...
server.registerTool(
  "index",
  {
    ...
    description:
      ...
      "name heuristic). Runs scip-typescript or scip-python via npx into .splus-cache/index.scip, " +
      "which `review` auto-detects. Occasional, not per-review — meant for a one-off or CI. " +
      "Requires the project's deps installed and that it typechecks.",
    inputSchema: {
      root: z.string().optional().describe("Repo root (default: server CWD)."),
    },
  ...
  async ({ root }) => {
    ...
    // Capture output — never inherit, since stdout is the MCP protocol channel.
    const r = spawnSync("npx", ["--yes", indexer, "index", "--output", out], {
      cwd: repo,
      encoding: "utf8",
    });
    if (r.status === 0) {
      return ok(`✓ Wrote ${join(repo, out)}. \`review\` will auto-detect it for precise blast radius.`);
    }
    const detail = `${r.stderr ?? ""}${r.stdout ?? ""}`.trim().slice(-1500);
    return fail(
      `Indexer (${indexer}) failed${r.status != null ? ` (exit ${r.status})` : ""}.\n${detail || "(no output)"}`,
    );
  },
);
...
async function main(): Promise<void> {
  ...
  process.stderr.write("splus-mcp ready (stdio) — tools: review, dismiss, mute, learnings, index\n");
}
